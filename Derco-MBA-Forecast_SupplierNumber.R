...
create_data <- function(original_datasource, blacklist, min.date, freq){
  ...
  ## paste Material Text and ID (not used for now)
  ...
  listOfRecent = listOfRecent %>% group_by(Year.Month.natural.Text, Supplier.Number) %>% tally()
  ...
  month.counts <- listOfRecent$Supplier.Number %>% table() %>% as.data.frame() %>% filter(Freq>=freq)
  
  ## keep all parts that were sold in x(freq) of last 30 months
  datasource<- datasource %>%
    filter(Supplier.Number %in% month.counts$.)
  
  ## gather baskets (long run time, maybe 2 minutes)
  datasource <- ddply(datasource,c("Invoice","Year.Month.natural.Text"),
                      function(df1)paste(df1$Supplier.Number,
                                         collapse = "|"))
  ## rename column from V1 to Supplier.Number
  colnames(datasource)[3] <- "Supplier.Number"
  
  listOfRecent = datasource %>% select(Year.Month.natural.Text, Supplier.Number) %>%
    filter(Year.Month.natural.Text >= min.date)
  listOfRecent = listOfRecent %>% group_by(Year.Month.natural.Text, Supplier.Number) %>% tally()
  ...
  ## Aggregates materials by month/year and Supplier.Number, get counts sold in that month
  datasource = datasource %>% group_by(Year.Month.natural.Text, Supplier.Number) %>% tally()

  ## return dataframe of 3 columns, and rows for every month for each material:  
  ## ds=date by months, y = sales by month, part = Supplier.Number
  data.frame("ds" = datasource$Year.Month.natural.Text,
             "y" = datasource$n, 
             "part" = datasource$Supplier.Number)
  }
...
filename <- paste(c('Datasets/Derco_Forecast_Baskets.freq_', freq,'SupplierNumber.csv'),collapse = '')
